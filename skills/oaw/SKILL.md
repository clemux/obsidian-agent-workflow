...
- With a real harness ID, lifecycle and `task note` writes append it to a deduplicated `session-ids` frontmatter list and preserve any legacy scalar `session-id`. The explicit missing-ID path writes only the body trace; it does not add a synthetic list value.
