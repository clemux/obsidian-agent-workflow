...
""",
    ...
    def test_task_review_rejects_blank_note_or_checks_without_writes(
        ...
        proc = self.run_oaw(
            "task", "review", "OAW-TSK-cli", "--note", note, "--checks", checks
        )
        self.assertEqual(proc.returncode, 1)
        self.assertIn(expected, proc.stderr)
        ...
        for note, checks, expected in (
            ...
            with pytest.raises(OawError, match=expected):
                lifecycle.update_task(
                    match, self.vault, "review", note, checks, allow_missing=True
                )

    @pytest.mark.parametrize(
        "initial_status", ["backlog", "todo", "active", "review", "done"]
    )
